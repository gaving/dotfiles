...
" OBJECT: g:FuzzyFinderMode.Snippet ----------------------------------------- {{{1
let g:FuzzyFinderMode.Snippet = copy(g:FuzzyFinderMode.Base)

function! g:FuzzyFinderMode.Snippet.on_complete(base)
  let patterns = self.make_pattern(a:base)
  let stats = self.get_filtered_stats(a:base)
  let result = self.find_snippet(patterns.re, s:SuffixNumber(patterns.base), self.enumerating_limit)
  return map(result, 's:SetRanks(v:val, v:val.word, a:base, stats)')
endfunction

function! g:FuzzyFinderMode.Snippet.on_open(expr, mode)
  exec "normal i" . a:expr . "\<c-r>=NERDSnippets_ExpandSnippet()\<cr>\<c-g>u\<c-r>=NERDSnippets_SwitchRegion(1)\<cr>"
endfunction

function! g:FuzzyFinderMode.Snippet.on_mode_enter_pre()
  let self.snippets = NERDSnippets_GetSnippets()
endfunction

function! g:FuzzyFinderMode.Snippet.find_snippet(pattern, index, limit)
  if !len(self.snippets)
    return []
  endif
  let key = join(self.snippets, "\n")
  call extend(self, { 'cache' : {} }, 'keep')
  if !exists('self.cache[key]')
    echo 'Caching snippet list...'
    let items = s:MapToSetSerialIndex(map(copy(self.snippets), '{ "word" : v:val }'), 1)
    let self.cache[key] = { 'items' : items }
  endif
  echo 'Filtering snippet list...'
...
let g:FuzzyFinderOptions = { 'Base':{}, 'Buffer':{}, 'File':{}, 'Dir':{}, 'MruFile':{}, 'MruCmd':{}, 'Bookmark':{}, 'Snippet':{}, 'Tag':{}, 'TaggedFile':{}}
"-----------------------------------------------------------------------------
...
"-----------------------------------------------------------------------------
" [Snippet Mode] This disables all functions of this mode if zero was set.
let g:FuzzyFinderOptions.Snippet.mode_available = 1
" [Snippet Mode] The prompt string.
let g:FuzzyFinderOptions.Snippet.prompt = '>Snippet>'
" [Snippet Mode] The highlight group name for a prompt string.
let g:FuzzyFinderOptions.Snippet.prompt_highlight = 'Question'
" [Snippet Mode] Pressing <BS> after a path separator deletes one directory name
" if non-zero is set.
let g:FuzzyFinderOptions.Snippet.smart_bs = 0
" [Snippet Mode] This is used to sort modes for switching to the next/previous
" mode.
let g:FuzzyFinderOptions.Snippet.switch_order = 70
" [Snippet Mode] The items matching this are excluded from the completion list.
let g:FuzzyFinderOptions.Snippet.excluded_path = '\v\~$|\.bak$|\.swp$'
"-----------------------------------------------------------------------------
...
command! -bang -narg=? -complete=tag    FuzzyFinderSnippet     call g:FuzzyFinderMode.Snippet.launch   (<q-args>, len(<q-bang>))
